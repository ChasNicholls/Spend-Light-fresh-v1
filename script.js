...
function importRulesFromFile(file) {
  ...
  reader.onload = () => {
    ...
    if (box) box.value = String(box.value).replace(/\n/g, '
').replace(/
/g, '
').replace(//g, '
');
...
/* === SpendLight Rules Newline Fix — appended by ChatGPT === */
(function () {
  const NL = '
';
  function normalizeRulesText(txt) {
  ...
  window.RulesNL = {
    ...
    normalizeBox() {
      const box = document.getElementById('rulesBox');
      if (!box) return;
      const cur = String(box.value || '');
      const fixed = normalizeRulesText(cur);
      if (cur !== fixed) box.value = fixed;
    },
    appendRuleLine(keyword, category) {
      const box = document.getElementById('rulesBox');
      if (!box) return;
      const line = String(keyword || '').toUpperCase() + ' => ' + String(category || '').toUpperCase();
      const cur = normalizeRulesText(String(box.value || '').trim());
      box.value = (cur ? cur + NL : '') + line + NL;
      box.scrollTop = box.scrollHeight;
    },
    textForExport() {
      const box = document.getElementById('rulesBox');
      const normalized = normalizeRulesText(box ? String(box.value || '') : '');
      return normalized.replace(/\n/g, '\r
');
    }
  };
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => RulesNL.normalizeBox());
  } else {
    RulesNL.normalizeBox();
  }
  document.addEventListener('paste', (e) => {
    if (e.target && e.target.id === 'rulesBox') {
      setTimeout(() => RulesNL.normalizeBox(), 0);
    }
  });
})(); 
/* === End Rules Newline Fix === */
